Tài liệu Đồ án Môn học SE360: Xây dựng
Nền tảng "UIT-Go" Cloud-Native
...
3. Giai đoạn 1: "Bộ Xương" Microservices (Bắt buộc
   ...
   3. DriverService:
      ○ Trách nhiệm: Quản lý trạng thái và vị trí của tài xế theo thời gian thực. Cung cấp API
      để tìm kiếm các tài xế phù hợp ở gần.
      ○ API gợi ý: PUT /drivers/{id}/location, GET /drivers/search?lat=...&lng=....
      ○ Công nghệ gợi ý: Node.js/Go/Python. Đối với dữ liệu vị trí, hãy phân tích và lựa chọn
      giữa hai hướng tiếp cận chính:
      ■ Ưu tiên tốc độ (Speed-first): Sử dụng Redis (ElastiCache) với tính năng
      Geospatial để tìm kiếm tài xế trong bán kính gần với độ trễ cực thấp.
      ■ Ưu tiên khả năng mở rộng và chi phí (Scale/Cost-first): Sử dụng DynamoDB
      kết hợp với Geohashing để tối ưu cho lượng ghi (write) vị trí khổng lồ và chi phí
